...
def help():
    ...
    roc.parse_coords(roc.orange_folder, roc.orange_number,
                     roc.orange_txt, roc.orange)
    roc.calculate_circle_radius(roc.orange_folder, roc.orange_number,
                                roc.orange, orange_data, result_list_of_coords_orange)
    roc.calculate_circle_radius(
        roc.grey_folder, roc.grey_number, roc.grey, grey_data, result_list_of_coords_grey)

    roc.parse_coords(roc.grey_folder, roc.grey_number_new,
                     roc.grey_txt, roc.grey, ".png", ".jpg")
    roc.parse_coords(roc.orange_folder, roc.orange_number_new,
                     roc.orange_txt, roc.orange, ".png", ".jpg")
    roc.calculate_circle_radius(roc.orange_folder, roc.orange_number_new,
                                roc.orange, orange_data, result_list_of_coords_orange, ".png")
    roc.calculate_circle_radius(roc.grey_folder, roc.grey_number_new,
                                roc.grey, grey_data, result_list_of_coords_grey, ".png")
...
def make_circles(cf, list_of_coords, radius_difference):
    getcontext().prec = 30  # точность вычислений 28 значащих цифр
    Circle = namedtuple('Circle', 'x y r w')
    # сколько сделать окружностей
    N_circles = np.random.randint(cf.N_circles_min, cf.N_circles_max)
    circles = []
    copied_circles = []
    for i in range(N_circles):

        # (cf.N_circles_min+cf.N_circles_min)//90)
        ...
        make_neighbourhood_circles(
            rnd_num, rnd_list, list_of_coords, radius_difference, fresh, copied_circles)
...
def rotate_point(point, angle, centr):
    ...
    rotated_translated_point = rotation_matrix @ np.array(
        [translated_x, translated_y])

    # Возвращаем точку к оригинальному положению
    rotated_point = (Decimal(rotated_translated_point[0] + Decimal(
        centr[0])), Decimal(rotated_translated_point[1] + Decimal(centr[1])))
...
def paste_image(cir, list_of_masks, image_folder, data, background, result_list_of_masks, result_count):
    ...
    # поворот капли
    # угол поворота картинки капельки отдельно
    rotateAngleSmallPicture = random.randrange(0, 360, 1)
    rotation_matrix = cv2.getRotationMatrix2D(
        (img_data[1], img_data[2]), -rotateAngleSmallPicture, 1.0)
    new_img = cv2.warpAffine(img, rotation_matrix, (100, 100))
    cv2.imwrite(image_folder+'/rotated_images/rotated_image.png', new_img)
    rotate = Image.open(os.path.join(
        image_folder, './rotated_images/rotated_image.png'))
    ...
    for x in range(0, len(mask_image)):
        rotated_point = rotate_point(
            mask_image[x], rotateAngleSmallPicture, (img_data[1], img_data[2]))
        new_mask_image.append(
            (int(point[0]+rotated_point[0]), int(point[1]+rotated_point[1])))
    ...
    intersection_percentage = (
        intersection.area / mask_image_polygon2.area) * 100
    flag_to_paste = False
...
def paste_mask(img_rgb):
    ...
    lineyka_rotate = lineyka.rotate(
        lineyka_rotate_angle, expand=False, resample=Image.BICUBIC)
...
def make_image_circles(circles, file_name, data, list_of_masks, rand_background_color=0):
    ...
    # -8,4 это отенки оранжевого    фильтр на фон
    hsv_const = random.randint(-8, 4)

    background = Image.new(
        'RGB', (2560, 1440), dict_rgb_circles_path[rand_background_color][0])  # цвет фона

    # путь до папки с каплями
    image_folder = dict_rgb_circles_path[rand_background_color][1]
    ...
    for cir in circles:  # вставляем капли
        paste_image(cir, list_of_masks, image_folder, data,
                    background, result_list_of_masks, result_count)

    # сохраняем в файл маски капель
    save_coords(file_name, result_list_of_masks)
    ...
    for num_image in range(cf.N_images):
        ...
        if rand_background_color == 1:
            circles = make_circles(
                cf, rand_background_color, result_list_of_coords_orange, 2)
            make_image_circles(cf, circles, f"circles_{num_image:04}.png", rand_background_color, orange_data, roc.orange, num_image,
                               )
        else:
            circles = make_circles(
                cf, rand_background_color, result_list_of_coords_grey, 3)
            make_image_circles(
                cf, circles, f"circles_{num_image:04}.png", rand_background_color, grey_data, roc.grey, num_image)
...
def generate_orange(cf):
    for num_image in range(cf.N_images):
        ...
        make_image_circles(
            circles, f"circles_{num_image:04}.png", orange_data, roc.orange, rand_background_color=1)
...
def generate_grey(cf):
    for num_image in range(cf.N_images):
        ...
        make_image_circles(
            circles, f"circles_{num_image:04}.png", grey_data, roc.grey)
