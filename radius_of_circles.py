...
def parse_coords(folder_name,list_numbers,folder_txt,dict):
    getcontext().prec = 28 
    for image_num in list_numbers:
        image_name=folder_name + f"/drop_{image_num}.png"
        image_txt=folder_txt + f"/drop_{image_num}.txt"
...
def calculate_circle_radius(folder_name,list_numbers,dict,result,list_of_coords):
    getcontext().prec = 28 
    for image_num in list_numbers:
        points = np.array(dict[folder_name + f"/drop_{image_num}.png"])
        ...
        result.update({result_radius:(f"./drop_{image_num}.png",int(float(center[0])),int(float(center[1])))})
